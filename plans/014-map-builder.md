...
- [x] **Step 10: Autotile terrain brush** `[delegate sonnet]`
  ...
  - Outcome: terrain defs live in a NEW sibling `public/assets/tilesets/pixel-crawler/terrains.json`
    (not pack.json — one entry per blob key would bury pack.json's hand-authored fields; decision
    documented in the file's `_comment`), one `grass` terrain, `fillFrame: 251`, 12-entry blob-key→
    frame `mapping` (`FULL_KEY 255`→251). Mapping is GENERATED by the new
    `scripts/pixel-crawler/gen_terrains.py` (reuses `build_blob`; `autotile.py` untouched, same rule
    as step 3), which converts Python's `(N,E,S,W,ne,se,sw,nw)` tuple to the TS bit-packed int
    (N=1<<7…NW=1<<0) and frame `r*25+c`. Asset catalog NOT touched (terrains load via their own
    fetch — `terrainCatalog.ts`/`terrainCatalogSource.ts` mirror `catalog.ts`/`catalogSource.ts`) so
    `asset-catalog.json` stays byte-identical; `npm run assets:catalog` not needed. New pure
    `src/editor/terrainOps.ts` (`computeTerrainBake` — reruns `paintMask` over the whole mask, diffs
    vs current layer cells, plus explicit clear-to-0 on erase). Store (`editorStore.ts`): `'terrain'`
    tool + `terrainCatalog`/`activeTerrainId`; `paintTerrainLine`/`fillTerrainFrom`/`paintTerrainRect`
    bundle mask + baked-cell (append-only palette) changes into ONE undoable command via
    `buildTerrainCommand` (lazy-materialize like `buildShapeCommand`; undo removes a freshly-created
    TerrainSection vs resets an existing one); `rebakeTerrainsForSave` full pre-save canonicalization.
    `EditorScene.ts`: `PaintTarget` +`'terrain'` routed through the existing generic
    `dispatchTargetPaint` (brush/rect/fill + Alt-erase, void-gated, like collision/zone/shape).
    `Toolbar.tsx`: Terrain tool + `handleSave` calls `rebakeTerrainsForSave()` before serialize.
    `LibraryPanel.tsx`: "🟩 Terrains" pseudo-category (sentinel + `TerrainCard` cropped-fill preview;
    arming switches to the terrain tool). `shortcuts.ts` updated (Terrain drag / Alt+drag).
    mapFormat.ts UNCHANGED (the `TerrainSection` + `MapFile.terrain` schema already existed from
    step 1). PYTHON-PARITY proof: `src/editor/__tests__/terrainOps.test.ts` asserts TS `paintMask` +
    the generated mapping reproduce a Python-computed fixture (`fixtures/grass-terrain-parity.json`,
    6×6 mask) exactly; +`editorStoreTerrainBrush.test.ts` (10 store integration tests incl.
    save→reopen→repaint round-trip). All 15 terrain tests green; full suite 499 tests. `npm run check`
    green except the pre-existing zero-diff `src/debug/crashReporter.ts` format failure (concurrent
    session, out of scope — same as steps 8/9). NOT machine-verified (no React/DOM harness): the live
    click-through at `npm run editor` — arm Grass in the Library's Terrains category, drag-paint a
    blob (edges/corners resolve), Alt+drag erases, undo reverts the whole stroke, save→reopen re-edits
    the same terrain seamlessly.

- [x] **Step 11: Game loads authored maps** `[inline]`
  - Outcome (leftovers only — core scope was landed/deployed by [plan 018](018-runtime-map-loader.md);
    see the Update block below): implemented the three genuinely-outstanding items 018/019 left open.
    (1) **Zones runtime read path** — new pure `src/systems/mapZones.ts`: `zoneAt(map,col,row)`
    (0 = no zone; `isInside`-guarded so OOB/void → 0) + `zoneDefAt(...)` resolving the `ZoneDef`;
    unit tests `src/systems/__tests__/mapZones.test.ts` (7). Exposed on the scene as
    `GameScene.zoneAt(col,row)` (converts GLOBAL→map-local by subtracting `mapOrigin`, mirroring
    `isBlocked`→`mapBlocks`) and via the DEV `__test` seam (`GameTestApi.zoneAt` in
    `src/entities/testTypes.ts`) so the data path is runtime-observable. Nothing consumes zones in
    gameplay yet — proves the path, as specced. (2) **Tier-1 world-integrity test**
    `src/data/maps/__tests__/world.test.ts`: eager-globs every committed `*.map.json` + `world.json`,
    `migrateMap` each, `validateWorld` across all, asserts zero errors, prints warnings (currently one:
    `test` unplaced — legitimate at L0). (3) **`MapConnections` placeholder** — added `MapConnection`
    type + empty `CONNECTIONS` const to `src/systems/mapRuntime.ts` (NOT the never-created
    `src/data/maps/index.ts` — 018 chose `mapRuntime.ts`; `gate?: unknown` until the future
    world-graph plan). Files touched: `mapZones.ts` (+test), `world.test.ts`, `mapRuntime.ts`,
    `GameScene.ts`, `testTypes.ts`. `npm run check` green except the pre-existing zero-diff
    `src/debug/crashReporter.ts` format failure (concurrent session, out of scope — same as steps
    8/9/10); full suite 509 tests pass. **Two 018 regressions scoped but DEFERRED, not fixed**
    (tracked debt): (a) `tests/e2e/survival-hunger.spec.ts`'s starve-HP assertion fails while
    `HUNGER_LETHAL=false` — intentionally coupled to that stopgap, which **step 12** unblocks by
    authoring real food/nodes into `test.map.json`; fix belongs there, not here. (b) `scripts/smoke.mjs`
    has a click-timing race against the now-async `PreloadScene.create()` — an independent test-infra
    fix, left for a follow-up. Not done here: the Tier-2 `?map=` e2e (018 cut the `?map=` param — no
    consumer) and any GameScene ground/object build work (018 owns it).
...
- [ ] **Step 12: Author the test maps, write docs, record decisions** `[inline]`
  - Progress (2026-07-15): **docs + decisions DONE; map-authoring DEFERRED to a hands-on editor
    session** (map authoring is visual/interactive — there is no headless authoring path, and
    hand-editing cells is forbidden by the format rules). Docs landed: new `docs/EDITOR.md` (run,
    panes, tools→`shortcuts.ts`, map vs world view, file format→`mapFormat.ts`/`worldLayout.ts` as
    source of truth, palette-append-only + void/shape rules, generated artifacts, persistence
    contract, packs→ASSETS.md, game loads via `mapRuntime.ts`); `docs/DECISIONS.md` — new
    2026-07-15 `[DECIDED]` Map Builder entry recording every outcome (palette encoding, src/data/maps
    home, zone layer, object discriminator, portals-in-maps/graph-in-registry, React+Phaser hybrid,
    baked-canonical autotiles, shape mask + world.json global coords, seamless walk-across +
    monsters-chase direction, error/warning split, seams-as-derived, lazy registry + manifest,
    committed thumbnails, fast-travel-needs-no-format-change, immutable-map + save-overlay contract,
    runtime-state-must-be-objects) and the old 2026-07-11 `[OPEN]` map-editor steer marked
    `[RESOLVED]` pointing at it; `docs/STATUS.md` — new plan-014 row (alongside the existing 017/018
    rows, not duplicating); `CLAUDE.md` — `docs/EDITOR.md` added to the docs index (architecture-map
    editor line already present). `npm run lint:md` 0 errors, prettier clean on all touched docs.
    **STILL OUTSTANDING (needs Matt at `npm run editor`):** author real content into
    `src/data/maps/test.map.json` (currently zero `node` objects) exercising every feature per the
    spec below; author `test-forest`; place both in `world.json` (interlocking shapes, ≥1 seam);
    verify each loads in-game. That work then unblocks: (1) flipping `HUNGER_LETHAL` back on
    (plan 018) once the map carries food/nodes; (2) plan 019's second-placement prerequisite; (3) the
    deferred `survival-hunger.spec.ts` fix (step 11). Also update `DECISIONS.md`'s registry entry /
    `GAME-DESIGN.md` map-editor tick only if still relevant after authoring.
