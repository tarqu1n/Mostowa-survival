...
export function ContextBar() {
  ...
  // The bar is a Map-tab tool surface. World has its own controls; object-editor tabs have none.
  ...
  const singleNode =
    selectedObjectIds.length === 1
      ...
      : undefined;
  const erase = PAINT_MODE_TOOLS.has(activeTool) ? ERASE_META[activeTool] : undefined;
      ...
      <div className={cn(groupClass, 'shrink-0')}>
        {underlay && (
          <Button
            variant="outline"
            size="icon-lg"
            aria-label={underlay.visible ? 'Hide reference underlay' : 'Show reference underlay'}
