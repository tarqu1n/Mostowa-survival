...
|2|`syncEnemyHealthBars` no-work guard|`CombatFxManager.ts:400-401`|Runs full body even with zero enemies|Early-return `if (enemies.length === 0 && this.hpBars.size === 0) return;` (must also gate on `hpBars.size` — else stale bars from enemies that died this frame are never destroyed, see `:432-438`)|safe|[fix]|
|3|`syncGlowTransforms` unconditional call|`TaskGlowRenderer.ts:187-195`; call `GameScene.ts:981`|Called every frame; Map-iterator allocation even when no harvest is queued (0 iterations)|`if (this.glowSprites.size === 0) return;`|safe|[fix]|
