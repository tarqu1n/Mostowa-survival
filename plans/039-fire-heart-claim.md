...
4. **Night is fully dark (black), and darkness conceals.** Raise `NIGHT_MAX_ALPHA` (`config.ts:345`)
...
5. **Light reveals with a soft radial gradient — via ERASE, not a mask, not a shader.** The current holes
...
6. **The player always emits a tiny personal light** (`PLAYER_LIGHT_RADIUS`, new — start small, ~1–1.5
   ...
   same seam).
7. **Claim light ≠ render light — never conflate; and claim uses the BRIGHT CORE.** The base **claim**
...
_Fresh-eyes review (2026-07-20). Verdict + severity table below; the plan above has been revised to
...
to a single rewired manager to right-size it._
